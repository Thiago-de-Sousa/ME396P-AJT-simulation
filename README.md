...
-----------------------------
Download and run the WindPowerCalc3000_History.py file located in this repository. Once it is running, you will be presented with the welcome interface. Keyboard commands will be
displayed with an explanation for each different command. 
...
Plots will only be generated if two turbine types are being compared, due to space limitations.
...
-----------------------------
Download and run the WindPowerCalc3000_Forecast.py file. The CSV file containing the turbine types and their respective parameters should already be located in the same directory.
The program will ask only for the desired location. Once the input is validated, the program will output a plot that shows the total power to be generated by all the turbines in the
farm at every moment for the next week.
