...
while turbine != 'q':
    ...
    if turbine == 'q':
        ...
        if plt:
            ...
            turb_name_forPlotSaving = turb_name.replace('/', '-')
            plt.title('Power Output for {} in {}'.format(turb_name_forPlotSaving, desired_loc))
            plt.xlabel('Time')
        ...
        for j in range(0, len(listOfDicts)):
            ccost = input('Enter the construction cost (USD) for a single {} turbine (scientific notation like 2.5e6 is ok): '.format(listOfDicts[j]['turbine_type']))
            if ccost == '0' or ccost == '':
                print("WARNING! Default value will be used...")
        ...
        if plt:
            ...
            if len(listOfDicts) == 2:
                ...
                plt.figure(3)
                plt.suptitle('{} - Financial Analysis Comparison'.format(desired_loc))
                plt.subplot(1, 2, 1)
                ...
                plt.figure(4)
                plt.suptitle('{} - Profit Comparison'.format(desired_loc))
                plt.subplot(1, 2, 1)
